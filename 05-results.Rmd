...
```{r, echo=FALSE, results='hide', message=FALSE, fig.width=4,fig.height=4}
housedata <- df
...
```{r, fig.width=20, echo=FALSE, results='hide', message=FALSE, fig.width=4,fig.height=4}

ggparcoord(housedata, columns = 4:8, alphaLines = .3,scale = "uniminmax") + 
  facet_wrap("Year") + 
  theme(axis.text.x=element_text(angle=45, hjust=1), axis.text=element_text(size=12),axis.title.x = element_text(size = 12)) + 
...
```{r, fig.height=15, echo=FALSE, results='hide', message=FALSE, fig.width=4,fig.height=4}
...
plot_usmap(data = unemployment, values = "unemployment", color = "white") +
  ...
  labs(title = "Unemployment by States", subtitle = 'Year 2010') +
...
plot_usmap(data =income, values = "income", color = "white") +
  facet_wrap(~year) +
   scale_fill_continuous(low = "mediumspringgreen", high = "red", name = "Income") +
...
plot_usmap(data = homevalue, values = "homevalue", color = "white", labels = FALSE) +
  facet_wrap(~year) +
   scale_fill_continuous(low = 'skyblue', high = 'red', name = "Homevalue") +
...
```{r, fig.height=15, echo=FALSE, results='hide', message=FALSE, fig.width=4,fig.height=4}
ggplot(housedata, aes(x=housedata$Income, y=housedata$Home.Value, color=housedata$State.Name)) + geom_point() + geom_smooth(method = "lm") + facet_geo(~ State.Name, grid = "us_state_grid2") +
  stat_cor(aes(label = ..rr.label..), color = "red", geom = "label")  + theme(legend.position = "none") +
  xlab("Income ($)") + ylab("Home Value ($)") + labs(title = "Home Value vs Income", subtitle = '2010 - 2019 across 50 States')
```
...
```{r, fig.height=10, echo=FALSE, results='hide', message=FALSE, fig.width=4,fig.height=4}
